...
class Profile(models.Model):
    ...
    def total_income(self):
        return self.transactions.filter(transaction_type="income").aggregate(sum=Sum("amount"))["sum"] or 0

    @property
    def total_outcome(self):
        return self.transactions.filter(transaction_type="outcome").aggregate(sum=Sum("amount"))["sum"] or 0
...
class Transaction(models.Model):
    ...
    user = models.ForeignKey(Profile, on_delete=models.PROTECT, related_name='transactions')
